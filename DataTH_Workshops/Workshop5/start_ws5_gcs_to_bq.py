...
from airflow.providers.google.cloud.transfers.gcs_to_bigquery import GCSToBigQueryOperator #มีการ import แล้ว
...
with DAG(
    ...
    # สร้าง t4 ที่เป็น GCSToBigQueryOperator เพื่อใช้งานกับ BigQuery แบบ Airflow และใส่ dependencies
    t4 = GCSToBigQueryOperator(
    task_id='gcs_to_bq',
    bucket='us-central1-workshop4-e25f0be1-bucket',  #มันก็จะเหมือนขั้นตอนที่แล้วแค่เขียนแยก us-central1-workshop4real-cc22592e-bucket/data/output.csv
    source_objects=['data/output.csv'],
    destination_project_dataset_table='workshop_5.audible_data2', #ให้ไปลงที่ Big Q ชื่อ workshop5 ชื่อว่า audible_data2
    skip_leading_rows = 1,
    schema_fields=[     #ในส่วน Schema_fields นั้นเอามาจากคำสั่ง bq show --schema --format prettyjson workshop5.audible_data
            {           #โดยทำการ run ใน terminal #ปกติแล้วเคสกรณีจริงเขาจะมีการเก็บเป็น .JSON ให้ จากนั้นเราก็ทำการ Load มาดู
                "mode": "NULLABLE",
                "name": "timestamp",
                "type": "TIMESTAMP"
            },
